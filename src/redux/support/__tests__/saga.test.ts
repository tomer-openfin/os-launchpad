...
describe('support/saga', () => {
  describe('watchSendBugRequest', () => {
    ...
    it('should put success action', async () => {
      const response = await apiMethod({ feedback: 'TEST_BUG' });

      const successClone = iterator.clone();

      expect(successClone.next(response).value).toEqual(put(success(undefined)));
  ...
  describe('watchSendFeedbackRequest', () => {
    ...
    it('should put success action', async () => {
      const response = await apiMethod({ feedback: 'TEST_FEEDBACK' });
